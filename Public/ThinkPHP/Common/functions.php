 ...
 * @return string
 */
function U($url='',$vars='',$suffix=true,$redirect=false,$domain=false,$script=null) {
    // 解析URL
    $info   =  parse_url($url);
    ...
    if(C('URL_MODEL') == 0) { // 普通模式URL转换
        $url        =   $script.'?'.http_build_query(array_reverse($var));
        if(!empty($vars)) {
        ...
        if(isset($route)) {
            $url    =   $script.'/'.rtrim($url,$depr);
        }else{
            $url    =   $script.'/'.implode($depr,array_reverse($var));
        }
        if(!empty($vars)) { // 添加参数
