...
class App {
    ...
    static public function init() {
        ...
        define('IS_AJAX',       ((isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') || !empty($_POST[C('VAR_AJAX_SUBMIT')]) || !empty($_GET[C('VAR_AJAX_SUBMIT')]) || !empty($_SERVER['HTTP_'. strtoupper(C('VAR_AJAX_SUBMIT'))])) ? true : false);
		define('IS_CROSSDOMAIN',(!empty($_GET[C('VAR_CROSSDOMAIN_SUBMIT')]) || !empty($_POST[C('VAR_CROSSDOMAIN_SUBMIT')])) ? true : false);
        ...
        if(C('TMPL_DETECT_THEME')) {// 自动侦测模板主题
            ...
            if (isset($_GET[$t])){
                ...
                $templateSet = cookie('template');
            }
            if(!in_array($templateSet,explode(',',C('THEME_LIST')))){
                $templateSet =  C('DEFAULT_THEME');
            }
            cookie('template',$templateSet,864000);
        }
        /* 模板相关目录常量 */
