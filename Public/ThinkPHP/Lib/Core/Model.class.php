...
class Model {
     ...
     * @example
     *
     * @param string $field	 进行分组的字段
     * @param boolean $muilt 每组是否允许多个
     * @param boolean $unset 是否注销所分组字段的值
     * @param array $options 见select()的参数配置说明
     *
     */
    public function groupBySelect($field = '', $muilt=false, $unset=true, $options = array()){
    	$datas = $this->select($options);
     ...
     * @return mixed
     */
    public function select($options=array(), $p_cache=true) {
        ...
        // 分析表达式
        $options    =  $this->_parseOptions($options);
        $resultSet  = $this->db->select($options, $p_cache);
        if(false === $resultSet) {
            return false;
        }
     ...
     * @return mixed
     */
    public function find($options=array(), $p_cache=true) {
        if(is_numeric($options) || is_string($options)) {
    ...
    public function regex($value,$rule) {
        $validate = array(
            ...
            'english'   =>  '/^[A-Za-z]+$/',//英文字母
			'ip' => '/^(\d+)\.(\d+)\.(\d+)\.(\d+)$/',//IP地址
			'image' => '/^.*\.(gif|jpg|jpeg|bmp|png)$/i',//文件格式是否为图片
			'phone_mobile' => '/((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)/',//固定电话或手机号
        );
        // 检查是否有内置的正则表达式
    ...
    public function page($page=null, $listRows=null){
		$p = (int)$_REQUEST[C('VAR_PAGE')];
		$page !== null OR $page = $p;
		!empty($page) OR $page = 1;
		$listRows !== null OR $listRows = C('PAGE_NUM');
