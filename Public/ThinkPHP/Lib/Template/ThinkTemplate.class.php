...
class  ThinkTemplate {
    ...
    public function parse($content) {
        ...
        // 检查include语法
		$content	= preg_replace('/\{include\s+(.*?)\}/i', '<include file="\\1"/>', $content);
		...
		$content = preg_replace('/\{:date\((.*?),(.*?)\)\}/i','<?php if (!empty(\\2)){ echo date(\\1, \\2); } ?>',$content);
		
		$content = preg_replace('/\{([A-Z_]+)\}/','<?php if(defined("\\1")) echo \\1; ?>',$content); //匹配替换全局变量
		
		//解析foreach语句
		$content = preg_replace('/\{foreach\s+(.*?)\}/i','<?php foreach(\\1): ?>',$content);
//		$content = preg_replace_callback('/\{foreach\s+([^\}]+?)\s+as([^}]+?)\}/si', 'ThinkTemplate::parseForeach',$content);
		$content = preg_replace_callback('/\{([^\{]*?)\$foreach\.([^\}^\.]+?)\.(first|last|index|count)([^\}]*?)\}/s','ThinkTemplate::parseForeachVar',$content);
	 ...
	 * 解析foreach标签
	 */
	private function parseForeach($var){
		$str = '<?php ';
	 ...
	 * 解析foreach奇、偶、总共个数标签
	 */
	private function parseForeachVar($var,$fix=true){
		$key = md5($var[2]);
		$str = '$this->_foreach[\''.$key.'\'][\'index\']';
