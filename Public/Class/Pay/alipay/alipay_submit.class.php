...
class AlipaySubmit {
	...
	function __construct($alipay_config){
		...
		if(TEMPLATE == 'web'){
			$this->alipay_gateway_new = 'https://www.alipay.com/cooperate/gateway.do?service=notify_verify';
		}
	}
	...
	function buildRequestForm($para_temp, $method, $button_name='') {
		...
		$sHtml = "<form id='pay_form' name='pay_form' action='".$this->alipay_gateway_new."_input_charset=".trim(strtolower($this->alipay_config['input_charset']))."' method='".$method."'>";
		while (list ($key, $val) = each ($para)) {
            $sHtml.= "<input type='hidden' name='".$key."' value='".$val."'/>";
        }

		//submit按钮控件请不要含有name属性
        $sHtml = $sHtml."<input type='submit' style='disply:none;' value='".$button_name."'></form>";
     ...
     * @param $p_url
     */
	function buildRequestFormGet($p_url) {
		$sHtml = "<form id='pay_form' name='pay_form' action='$p_url' method='get'>";
		//submit按钮控件请不要含有name属性
        $sHtml = $sHtml."<input type='submit' style='disply:none;' value='立即支付'></form>";
