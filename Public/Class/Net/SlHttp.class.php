...
class SlHttp {
    ...
    private $crlf = "\r\n";//请求分隔符
    private $accept = "*/*";
    private $agent = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.12; rv:53.0) Gecko/20100101 Firefox/53.0';//伪装浏览器
    ...
    public function sendRequest($url, $data=array(), $method='post') {
		...
		if (isset($this->header['X-Requested-With'])) {
			$headerStr .= "X-Requested-With: {$this->header['X-Requested-With']}{$this->crlf}";
		}
		if (isset($this->header['DNT'])) {
			$headerStr .= "DNT: {$this->header['DNT']}{$this->crlf}";
		}
        $dataLength = 0;
        if ($method == 'POST') {            
            if (!empty($data)) {
